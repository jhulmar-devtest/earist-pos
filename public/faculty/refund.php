...
// Fetch order belonging to this faculty member
$stmt = $db->prepare(
  "SELECT o.*, p.payment_status
   FROM orders o
   LEFT JOIN payments p ON o.id = p.order_id
   WHERE o.id = ? AND o.faculty_id = ?"
);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (empty($reason)) {
    ...
    redirect(APP_URL . '/faculty/refund.php?order=' . $oid);
  }
  ...
  // Reuse refund_requests table — store faculty id in student_id column
  $db->prepare(
    "INSERT INTO refund_requests (order_id, student_id, reason) VALUES (?, ?, ?)"
  )->execute([$oid, $uid, $reason]);
  ...
  <div class="card">
    <div class="card-body">
      <div class="alert alert-info mb-4">
        ...
        <div>
          Refund for order <strong><?= e($order['order_number']) ?></strong> — <?= peso($order['total_amount']) ?>.
      ...
      <form method="POST">
        ...
        <div class="form-group">
          <label class="form-label">
            Reason for Refund <span style="color:var(--status-cancelled)">*</span>
          </label>
          <textarea name="reason" class="form-control" rows="4" required
            placeholder="Explain why you are requesting a refund…"></textarea>
        </div>
        <div style="display:flex;gap:10px">
          <a href="<?= APP_URL ?>/faculty/orders.php" class="btn btn-ghost">Cancel</a>

...
// ── AJAX: return order items as JSON ─────────────────────────
if (isset($_GET['get_order_items'])) {
  ...
  // Verify order belongs to current faculty
  $chk = $db->prepare("SELECT id FROM orders WHERE id=? AND faculty_id=?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_feedback'])) {
  ...
  if ($cashierRating < 1 || $cashierRating > 5) {
    ...
    redirect(APP_URL . '/faculty/orders.php');
  }

  // Verify order belongs to faculty and is claimed
  $stmt = $db->prepare(
    "SELECT o.id, o.cashier_id FROM orders o
     WHERE o.id = ? AND o.faculty_id = ? AND o.status = 'claimed'"
  );
  ...
  try {
    ...
    $db->prepare(
      "INSERT INTO order_feedback (order_id, faculty_id, cashier_id, rating, comment)
       VALUES (?, ?, ?, ?, ?)"
    )->execute([$orderId, currentUserId(), $order['cashier_id'] ?: null, $cashierRating, $comment ?: null]);

    $feedbackId = (int)$db->lastInsertId();

    // Insert product ratings only if they exist
    if (!empty($productRatings)) {
    $pstmt = $db->prepare(
  "INSERT INTO product_ratings (feedback_id, order_id, product_id, faculty_id, rating)
   VALUES (?, ?, ?, ?, ?)"
);
...
foreach ($productRatings as $pid => $prating) {
    $pid     = (int)$pid;
    $prating = (int)$prating;
    if (!in_array($pid, $validProductIds, true) || $prating < 1 || $prating > 5) continue;

    // Faculty feedback: student_id left NULL, faculty_id filled
  ...
  redirect(APP_URL . '/faculty/orders.php');  // ← NOW properly inside the if block
} 
...
$where  = '';
if ($filter !== 'all') { $where = ' AND o.status = ?'; $params[] = $filter; }
...
<style>
  ...
  .star-group label { font-size:1.80rem; color:var(--border-strong); cursor:pointer; transition:color var(--transition-fast); line-height:1; }
  .star-group input:checked~label,.star-group label:hover,.star-group label:hover~label { color:var(--accent-color); }

  .product-star-group { display:flex; flex-direction:row-reverse; gap:2px; justify-content:flex-end; }
  .product-star-group input { display:none; }
  .product-star-group label { font-size:1.20rem; color:var(--border-strong); cursor:pointer; transition:color var(--transition-fast); line-height:1; }
  .product-star-group input:checked~label,.product-star-group label:hover,.product-star-group label:hover~label { color:var(--accent-color); }

  .product-rate-card { display:flex; align-items:center; gap:var(--space-3); padding:var(--space-3); border:1.5px solid var(--border-color); border-radius:var(--radius-sm); background:var(--surface-raised); margin-bottom:var(--space-2); transition:border-color var(--transition-fast); }
  .product-rate-card:has(.product-star-group input:checked) { border-color:var(--accent-color); background:var(--accent-subtle); }
  .product-rate-thumb { width:44px; height:44px; flex-shrink:0; border-radius:var(--radius-xs); overflow:hidden; background:var(--surface-sunken); border:1px solid var(--border-color); display:flex; align-items:center; justify-content:center; }
  .product-rate-thumb img { width:100%; height:100%; object-fit:cover; }
  .product-rate-thumb i { font-size:16px; color:var(--border-strong); }
  .product-rate-info { flex:1; min-width:0; }
  .product-rate-name { font-size:0.82rem; font-weight:700; color:var(--text-color); overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
  .product-rate-note { font-size:0.68rem; color:var(--text-muted); margin-top:1px; }

  .fb-section-label { font-size:0.68rem; font-weight:800; text-transform:uppercase; letter-spacing:0.10em; color:var(--text-muted); margin:var(--space-4) 0 var(--space-2); display:flex; align-items:center; gap:var(--space-3); }
  .fb-section-label::after { content:''; flex:1; height:1px; background:var(--border-color); }

  .star-display { color:var(--accent-color); font-size:0.80rem; letter-spacing:1px; }
  .star-display .empty { color:var(--border-strong); }

  .btn-rate { height:28px; padding:0 10px; border:1.5px solid var(--accent-color); border-radius:var(--radius-full); background:var(--accent-subtle); color:var(--accent-dark); font-size:0.72rem; font-weight:700; font-family:inherit; cursor:pointer; transition:all var(--transition-fast); display:inline-flex; align-items:center; gap:4px; }
  .btn-rate:hover { background:var(--accent-color); color:var(--text-on-accent); }
</style>
...
<div class="page-header">
  ...
  <div class="page-header-actions">
    <a href="<?= APP_URL ?>/faculty/menu.php" class="btn btn-primary btn-sm">
          ...
          <tr>
            <td colspan="7">
              <div class="empty-state">
                ...
                <h3>No orders yet</h3>
                <p><a href="<?= APP_URL ?>/faculty/menu.php" style="color:var(--primary-color);font-weight:600">Browse the menu →</a></p>
              </div>
            </td>
          </tr>
    ...
    <form method="POST" id="feedback-form">
      ...
      <div class="modal-body" style="max-height:70vh;overflow-y:auto">
        ...
        <div class="form-group" style="text-align:center">
          <label class="form-label" style="display:block;margin-bottom:var(--space-3)">How was the service?</label>
        ...
        <div class="form-group">
          ...
          <textarea name="comment" class="form-control" rows="2" placeholder="Tell us about your order — quality, speed, service…"></textarea>
        </div>
      </div>
...
<script>
  ...
  async function openFeedback(orderId, orderNo) {
    ...
    try {
      const res  = await fetch('<?= APP_URL ?>/faculty/orders.php?get_order_items=' + orderId + '&csrf=<?= csrfToken() ?>');
      const data = await res.json();
      if (data.items && data.items.length > 0) { renderProductCards(data.items); document.getElementById('fb-products-section').style.display = 'block'; }
    } catch(e) {}
  }
  ...
  function renderProductCards(items) {
    ...
    items.forEach(item => {
      ...
      const img  = item.image_path ? `<img src="${productImgBase}${item.image_path}" alt="${name}">` : `<i class="fa-solid fa-mug-hot"></i>`;
      let stars = '';
      for (let s = 5; s >= 1; s--) {
  ...
  document.querySelectorAll('#star-group input').forEach(radio => {
    radio.addEventListener('change', () => { document.getElementById('star-label').textContent = starLabels[radio.value] || ''; });
  });

  document.getElementById('feedback-modal').addEventListener('click', e => { if (e.target === e.currentTarget) closeFeedback(); });
</script>
<?php layoutFooter(); ?>

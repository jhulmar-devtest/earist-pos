...
$activeOrders = $db->prepare("SELECT * FROM orders WHERE student_id=? AND status IN ('pending','preparing','ready') ORDER BY created_at DESC");
...
<div class="page-header">
  <div>
    ...
    <div class="page-header-sub"><?= date('l, F j') ?> · <?= count($active) > 0 ? count($active) . ' order' . (count($active) !== 1 ? 's' : '') . ' active' : 'No active orders' ?></div>
  </div>
  <div class="page-header-actions">
    <a href="<?= APP_URL ?>/student/menu.php" class="btn btn-primary">
      ...
      <div class="card-body">
        ...
        <?php if ($ao['status'] === STATUS_READY): ?>
          <div class="alert alert-success" style="margin-bottom:0">
            ...
            <div><strong>Your order is ready!</strong> Go to the counter and present your <strong>school ID</strong> to claim it.</div>
          </div>
...
<?php else: ?>
  <div class="card">
    <div class="empty-state">
      ...
      <a href="<?= APP_URL ?>/student/menu.php" class="btn btn-primary mt-3">

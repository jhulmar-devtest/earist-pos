    ...
    try {
      ...
      if (!empty($denoms)) {
        ...
        foreach ($denoms as $d => $qty) {
          $dstmt->execute([$paymentId, (float)$d, $qty]);
        }
      }
      $db->commit();
...
$allProducts = $db->query(
  "SELECT p.*, c.name AS cat_name, c.id AS cat_id,
     ...
     GROUP BY p.id, c.name, c.id, c.parent_id, cp.id, c.sort_order
     ORDER BY c.sort_order, p.name"
)->fetchAll();

// Best-seller threshold: top-5 by units sold
$_sold = array_column($allProducts, 'total_sold');
rsort($_sold);
$bestSellerThreshold = ($_sold[0] ?? 0) > 0 ? ($_sold[min(4, count($_sold) - 1)] ?? 1) : PHP_INT_MAX;
...
// Load add-ons for the customisation modal
$addons = $db->query(
...
<style>
  ...
  /* Single scrollable body: items + total + denominations */
  .pos-cart-body {
    flex: 1;
    overflow-y: auto;
    overflow-x: hidden;
  ...
  /* Items sit inside the body — no fixed height */
  .pos-cart-items {
    flex-shrink: 0;
  }
  ...
  .pos-item {
    display: grid;
    grid-template-columns: 40px 44px 1fr 56px;
    align-items: center;
    gap: var(--space-2);
    padding: 8px 12px;
    border-bottom: 1px solid var(--sidebar-border);
  ...
  .pos-change-amount.ok {
    color: var(--status-ready);
  }

  .pos-change-amount.short {
    color: var(--status-cancelled);
  }
  ...
  <div class="pos-menu">
    ...
    <div class="filter-bar-wrap" id="filter-bar-wrap">
      <div class="cat-pills" id="tier-1">
        <button class="cat-pill active" data-type="all" data-id="all">All</button>
          ...
          <div class="pos-card-img">
            ...
            <?php if (!$p['is_available']): ?>
              <div style="position:absolute;inset:0;background:rgba(0,0,0,0.45);display:flex;align-items:center;justify-content:center">
                <span style="background:var(--status-cancelled);color:var(--text-on-primary);font-size:0.58rem;font-weight:700;padding:3px 8px;border-radius:var(--radius-full);text-transform:uppercase;letter-spacing:0.06em">Unavailable</span>
              </div>
            <?php endif; ?>
  ...
  <div class="pos-cart">
    ...
    <div class="pos-cart-head">
      <div class="pos-cart-title">
        <i class="fa-solid fa-receipt"></i> Current Order
      </div>
      <div style="display:flex;align-items:center;gap:var(--space-2)">
        <span class="pos-item-chip" id="pos-item-chip">Empty</span>
        <button style="background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.10);border-radius:var(--radius-xs);width:26px;height:26px;display:flex;align-items:center;justify-content:center;cursor:pointer;color:var(--sidebar-text-dim);font-size:11px;transition:all var(--transition-fast)"
          onclick="clearCart()" title="Clear order"
          onmouseover="this.style.background='rgba(220,38,38,0.15)';this.style.color='var(--status-cancelled)'"
          onmouseout="this.style.background='rgba(255,255,255,0.06)';this.style.color='var(--sidebar-text-dim)'">
...
<script>
  ...
  ══════════════════════════════════════════════════════ */
  let cart = {};
  let _discountType = 'none'; // 'none' | 'pwd' | 'senior'
  ...
  if (cash > 0 && Object.keys(cart).length > 0) {
    ...
    if (diff >= 0) {
      amountEl.textContent = '\u20b1' + diff.toFixed(2);
      amountEl.className = 'pos-change-amount ok';
      processEl.disabled = false;
    } else {
      amountEl.textContent = '\u2212\u20b1' + Math.abs(diff).toFixed(2) + ' short';
      amountEl.className = 'pos-change-amount short';
      processEl.disabled = true;
    }
  } else {
    changeEl.classList.remove('visible');
    processEl.disabled = Object.keys(cart).length === 0;
  }

  /* ── 2-Tier Filter / search ─────────────────── */
  ...
  document.getElementById('tier-1').addEventListener('click', e => {
    ...
    if (_activeType === 'group') {
      let hasChildren = false;
      subPills.forEach(sp => {
        if (sp.dataset.parent === _activeId) {
  ...
  document.getElementById('tier-2').addEventListener('click', e => {
    ...
    if (_activeId === pill.dataset.id && _activeType === 'subcat') {
      _activeType = 'group';
      _activeId = pill.dataset.parent;
    } else {
      pill.classList.add('active');
      _activeType = 'subcat';
      _activeId = pill.dataset.id;
  ...
  function applyFilter() {
    ...
    document.querySelectorAll('#pos-grid .pos-card').forEach(card => {
      let catMatch = false;
      if (_activeType === 'all') catMatch = true;
      else if (_activeType === 'group') catMatch = card.dataset.parent === _activeId;
      else if (_activeType === 'cat' || _activeType === 'subcat') catMatch = card.dataset.cat === _activeId;
  ...
  function submitOrder() {
    ...
    const cash = _denomCashTotal;
    if (cash < fin.total) {
      showToast('Cash received is less than the total!', 'error');
      return;
    }
  ...
  const POS_ADDONS = <?php echo json_encode(array_map(fn($a) => [
                        'id' => $a['id'],
                        'name' => $a['name'],
                        'price' => (float)$a['price']
                      ], $addons)); ?>;
  ...
  // Groups that support size + sugar + add-ons  (Coffees=5, Other Drinks=6)
  const DRINK_GROUPS = [5, 6];

  function openPosModal(id, name, basePrice, imgPath, groupId) {
    const isDrink = DRINK_GROUPS.includes(Number(groupId));
    _pos = {
      id,
      name,
    ...
    // Show/hide customisation sections based on category group
    document.getElementById('pm-section-size').style.display = isDrink ? '' : 'none';
    document.getElementById('pm-section-sugar').style.display = isDrink ? '' : 'none';
    document.getElementById('pm-section-addons').style.display = isDrink ? '' : 'none';
  ...
  function posAddToCart() {
    ...
    const size = isDrink ? (POS_SIZES[sizeIdx]?.label ?? 'Medium') : null;
    const sizeAdj = isDrink ? (POS_SIZES[sizeIdx]?.adj ?? 0) : 0;
    const sugar = isDrink ? (POS_SUGAR[sugarIdx] ?? 'Full Sugar') : null;
    const qty = Math.max(1, parseInt(document.getElementById('pm-qty').value) || 1);
    ...
    // For drinks: key includes size+sugar+addons so different customisations = separate rows
    // For food/retail: key is just product id so same product stacks in one row
    const key = isDrink ?
      _pos.id + '|' + size + '|' + sugar + (chosenAddons.length ? '|' + chosenAddons.join(',') : '') :
      _pos.id + '|plain';

    const noteParts = [];
    if (isDrink) {
      noteParts.push(size, sugar);
